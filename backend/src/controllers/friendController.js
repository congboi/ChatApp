...
export const sendFriendRequest = async (req,res) => {
    try {
        ...
        if(alreadyFriends){
            return res.status(400).json({message: "Bạn đã là bạn bè của nhau"});
        }

        if(existingRequest){
            return res.status(400).json({message: "Bạn đã gửi lời mời kết bạn cho người này đang chờ"});
        }
...
export const acceptFriendRequest = async (req,res) => {
    try {
        ...
        if(request.to.toString() !== userId.toString()){
            return res.status(403).json({message:"Không có quyền chấp nhận lời mời kết bạn này"});
        }
        ...
        return res.status(200).json({message: "Đã chấp nhận lời mời kết bạn", newFriend:{
            _id: from?._id,
...
export const declineFriendRequest = async (req,res) => {
    try {
        ...
        if(request.to.toString() !== userId.toString()){
            return res.status(403).json({message:"Không có quyền từ chối lời mời kết bạn này"});
        }
